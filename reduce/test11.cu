...
void __global__ reduce_shfl(int *d_x, int *d_y, int N)
{
	...
	for (int off = 16; off > 0; off >>=1) {
		y += __shfl_down_sync(FULL_MASK, y, off);
	}
...
void __global__ reduce_cp(int *d_x, int *d_y, int N)
{
        const int tid = threadIdx.x;
        const int bid = blockIdx.x;
        const int n = bid * blockDim.x + tid;
...
int main(void) {
	...
	reduce_cp<<<grid_size, block_size, SMEM>>>(d_A, d_B, N);
